...
type bench struct {
	setup func(*testing.B, *gmap.Map[int, int])
	perG  func(b *testing.B, pb *testing.PB, i int, m *gmap.Map[int, int])
}

func benchMap(b *testing.B, bench bench) {
	for _, m := range [...]*gmap.Map[int, int]{{}} {
...
func BenchmarkLoadMostlyHits(b *testing.B) {
	...
	benchMap(b, bench{
		setup: func(_ *testing.B, m *gmap.Map[int, int]) {
			for i := 0; i < hits; i++ {
				m.LoadOrStore(i, i)
			}
...
func BenchmarkLoadOrStoreBalanced(b *testing.B) {
	...
	benchMap(b, bench{
		setup: func(b *testing.B, m *gmap.Map[int, int]) {
			for i := 0; i < hits; i++ {
				m.LoadOrStore(i, i*2)
			}
			// Prime the map to get it into a steady state.
