...
func (c mapCall) apply(m mapInterface) (string, bool) {
	switch c.op {
	case opLoad:
		return m.Load(c.k)
	case opStore:
...
func applyMap(calls []mapCall) ([]mapResult, map[any]any) {
	return applyCalls(new(gmap.Map[string, string]), calls)
}
...
func TestMapRangeNestedCall(t *testing.T) { // Issue 46399
	...
	if length != 0 {
		t.Fatalf("Unexpected gmap.Map size, got %v want %v", length, 0)
	}
}

func TestMapLoadFaild(t *testing.T) {
	...
	if j < hits {
		if _, ok := m.LoadOrStore(j, i); !ok {
			t.Fatalf("unexpected miss for %v", j)
		}
	} else {
		if v, loaded := m.LoadOrStore(i, i); loaded {
			t.Fatalf("failed to store %v: existing value %v", i, v)
		}
	}
}
